...
first_pass_regression <- quantifs |>
  ...
  select(-fit)
# qs::qsave(first_pass_regression, "data/intermediates/230117_fits_for_quantifs_filt_scaled_log_cache.qs")


first_pass_regression <- qs::qread("data/intermediates/230117_fits_for_quantifs_filt_scaled_log_cache.qs")
...
cvfit <- cv.glmnet(x[train,], y[train], nfolds = 30, type.measure = "mae")
plot(cvfit)
