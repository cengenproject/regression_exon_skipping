...
regression_lasso <- function(my_ev, mat_sf_expression, quantifs){
...
regression_adalasso <- function(my_ev, mat_sf_expression, quantifs){
...
first_pass_regression <- quantifs_filtered |>
  ...
  mutate(fit_lasso = map(event_id,
                         \(ev) regression_lasso(ev, mat_sf_expression, quantifs),
                         .progress = TRUE),
         fit_adalasso = map(event_id,
                            \(ev) regression_adalasso(ev, mat_sf_expression, quantifs),
                            .progress = TRUE)) |>
  mutate(lasso_rsquare            = map_dbl(fit_lasso, pluck("rsquare")),
         lasso_nb_coefs           = map_int(fit_lasso, pluck("nb_coefs")),
         lasso_prediction_on_test = map(fit_lasso, pluck("prediction_on_test")),
         lasso_coefs_sf           = map(fit_lasso, pluck("coefs_sf")),
         adalasso_rsquare            = map_dbl(fit_adalasso, pluck("rsquare")),
         adalasso_nb_coefs           = map_int(fit_adalasso, pluck("nb_coefs")),
         adalasso_prediction_on_test = map(fit_adalasso, pluck("prediction_on_test")),
         adalasso_coefs_sf           = map(fit_adalasso, pluck("coefs_sf")))
# qs::qsave(first_pass_regression, "data/intermediates/230123_fits_for_quantifs_cache.qs")


first_pass_regression <- qs::qread("data/intermediates/230123_fits_for_quantifs_cache.qs")
...
patchwork::wrap_plots(
  ggplot(first_pass_regression,
         aes(x = nb_samples, y = lasso_rsquare)) +
    theme_classic() +
    geom_point(alpha = .2) +
    geom_smooth(),
  ggplot(first_pass_regression,
         aes(x = nb_samples, y = lasso_nb_coefs)) +
    theme_classic() +
    geom_point(alpha = .2) +
    geom_smooth(),
  ggplot(first_pass_regression,
         aes(x = nb_samples, y = adalasso_rsquare)) +
    theme_classic() +
    geom_point(alpha = .2) +
    geom_smooth(),
  ggplot(first_pass_regression,
         aes(x = nb_samples, y = adalasso_nb_coefs)) +
    theme_classic() +
    geom_point(alpha = .2) +
    geom_smooth(),
  ncol = 2
