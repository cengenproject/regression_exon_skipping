...
plot(fit)
# log(fit$lambda) |> setNames(fit$lambda)
...
left_join(
  ...
  left_join(
    sim_sf |>
      filter(transcript_id == "F42A6.7c.1") |>
...
# distribute covariance: S' = CxS
coefs_sf$Sp <- cov(mat_sf_expression) %*% coefs_sf$s1

full_join(coefs_sf |> select(transcript_id, computed = Sp),
          true_coefs |> select(transcript_id, ground_truth = true_coef),
          by = "transcript_id") |>
  ggplot() + theme_classic() +
  geom_vline(aes(xintercept = 0), color = 'grey') +
...
reg_lasso1 <- regression_wrapper(my_ev = my_ev,
                                 regression_method = "adaptive_lasso",
                                 column = "dPSI_nat",
                                 shuffle = FALSE,
...
# Reproducibility ----
(my_ev <- sample(events_to_keep, 1))
reg_lasso_repl <- map(1:50,
                      ~ regression_wrapper(my_ev = my_ev,
...
reg_lasso_repl_transp <- transpose(reg_lasso_repl)
plot(unlist(reg_lasso_repl_transp$nb_coefs))
xx <- reg_lasso_repl_transp$coefs_sf |>
  set_names(paste0("rep_", 1:50)) |>
  imap(~ add_column(.x, rep = .y)) |>
  list_rbind() |>
  group_by(transcript_id) |>
  summarize(mean_s1 = median(s1))

full_join(xx |> select(transcript_id, computed = mean_s1),
          true_coefs |> select(transcript_id, ground_truth = true_coef),
          by = "transcript_id") |>
  ggplot() + theme_classic() +
  geom_vline(aes(xintercept = 0), color = 'grey') +
...
xx <- reg_lasso_repl |>
  map(~ {
    .x$coefs_sf$Sp <- c(0, cov(mat_sf_expression) %*% .x$coefs_sf$s1[-1])
    .x
  }) |>
  transpose()
xx2 <- xx$coefs_sf |>
  ...
  list_rbind() |>
  group_by(transcript_id) |>
  summarize(mean_s1 = mean(s1),
            mean_sp = mean(Sp))


full_join(xx2 |> select(transcript_id, computed = mean_sp),
          true_coefs |> select(transcript_id, ground_truth = true_coef),
          by = "transcript_id") |>
  ggplot() + theme_classic() +
  geom_vline(aes(xintercept = 0), color = 'grey') +
