...
regression_multinomial_lasso <- function(my_ev, mat_sf_expression, quants){
  ...
  prediction_on_test <- tibble(
    sample_id = rownames(prediction_array),
...
reg_multilasso <- regression_multinomial_lasso(my_ev, mat_sf_expression, quantifs_filtered)
  ...
  mutate(fit_lasso = map(event_id,
         ...
         fit_multlasso = map(event_id,
                            \(ev) possibly(regression_multinomial_lasso,
                                           otherwise = list(NA))(ev, mat_sf_expression, quantifs_filtered),
                            .progress = TRUE))
# qs::qsave(first_pass_regression, "data/intermediates/230124_fits_for_quantifs_4methods_cache.qs")


first_pass_regression <- qs::qread("data/intermediates/230124_fits_for_quantifs_4methods_cache.qs")
