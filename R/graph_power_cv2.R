...
# DO NOT USE: instead use graph_power_cv2bis which properly separates NPN and imputation
...
# QUIC ----
res_quic <- QUIC::QUIC(cov(mat_train), rho = .1)
...
Y_psi <- t(mat_train[,(nb_sf+1):(nb_sf+nb_psi)])
Y_sf <- t(mat_train[,1:nb_sf])
...
vald_psi <- t(mat_train[folds == 1, (nb_sf+1):(nb_sf+nb_psi)])
