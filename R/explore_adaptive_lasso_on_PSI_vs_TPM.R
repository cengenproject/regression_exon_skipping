...
# Same as explore_lasso_on_PSI_vs_TPM, but with adaptive lasso
...
sparse_regression <- function(my_ev){
  ...
  cvfit <- adaLasso(x[train,], y[train], nfolds = 30)
...
first_pass_regression <- quantifs |>
  ...
  select(-fit)
# qs::qsave(first_pass_regression, "data/intermediates/230118_fits_for_quantifs_adalasso_cache.qs")


first_pass_regression <- qs::qread("data/intermediates/230118_fits_for_quantifs_adalasso_cache.qs")
...
# writeLines(events_to_keep, "data/intermediates/230119_events_to_keep.txt")
...
cvfit <- adaLasso(x[train,], y[train], nfolds = 30, type.measure = "mae")
plot(cvfit)
plot(cvfit$glmnet.fit, xvar = "lambda"); abline(v = c(log(cvfit$lambda.1se),
...
cvfit <- adaLasso(x[train,], y[train])
plot(cvfit)
plot(cvfit$glmnet.fit, xvar = "lambda"); abline(v = c(log(cvfit$lambda.1se),
                                                      log(cvfit$lambda.min)),
                                                lty = 'dashed')
...
sum(overlaps_sf$nb_computed_sf)
126/1496
...
#> 12% on log scale
225/1741
#> 13% with proper filtering of samples (then log and scaling)
342/1741
#> 20% with adaptive LASSO
...
hist(randomized_overlaps, breaks = 30) ; abline(v = sum(overlaps_sf$nb_overlapping_sf), col = 'darkred')
...
replicated_regression <- pbapply::pbreplicate(n = 50,
                                   ...
                                   simplify = FALSE)
# qs::qsave(replicated_regression, "data/intermediates/230118_replicated_regression_adalasso_cache.qs")


replicated_regression <- qs::qread("data/intermediates/230118_replicated_regression_adalasso_cache.qs")
...
ggplot(coefs_stability_nonnull) +
  theme_classic() +
  geom_point(aes(x = abs(mean), y = sd)) +
  scale_x_log10() + scale_y_log10() +
  geom_abline(aes(intercept = 0, slope = 1))


plot(x = log10(abs(coefs_stability_nonnull$mean)), y = log10(coefs_stability_nonnull$sd))



plot(x = coefs_stability_nonnull$mean, y = coefs_stability_nonnull$sd)
plot(x = coefs_stability_nonnull$median, y = coefs_stability_nonnull$mad)
plot(x = log1p(coefs_stability$median), y = log1p(coefs_stability$mad))
plot(x = coefs_stability_nonnull$median, y = log10(coefs_stability_nonnull$mad))

abline(coef = c(0,1))

lm(log10(sd) ~ log10(mean), data = coefs_stability)

table(coefs_stability$mean > 0.2, coefs_stability$median > 0.2)
table(coefs_stability$mean > 0.2, coefs_stability$sd < 0.2, dnn = c("mean", "sd"))
plot(x = coefs_stability$mean, y = coefs_stability$sd)
abline(v = .2, col ='darkred', lty = 'dotted')
abline(h = .1, col ='darkred', lty = 'dotted')


table(coefs_stability$mean > 0.2, coefs_stability$sd < 0.1, dnn = c("mean > 0.2", "sd < 0.1")) |>
  addmargins()


#~ Compare with known ----

intersected_stable_coefs <- coefs_stability |>
  filter(mean > 0.2,
         sd < 0.1) |>
  # add event info
  left_join(events_coordinates |>
...
overlap_intersected_stable_coefs <- intersected_stable_coefs |>
  select(event_id, target_id, target_name) |>
  add_column(nb_sf_overlap = map2_int(intersected_stable_coefs$computed, intersected_stable_coefs$known,
                                      ~ length(intersect(.x$computed_sf_gene_id, .y$sf_id))),
             nb_sf_computed = map_int(intersected_stable_coefs$computed, ~length(.x$computed_sf_gene_id) - 1),
             nb_sf_known = map_int(intersected_stable_coefs$known, ~length(.x$sf_id)))
